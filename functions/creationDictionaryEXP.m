...
switch setting.frameSize
    ...
    otherwise   % W composed of several temporal window get by the rms of the selected window
        ...
        [~,idxBeg] = min(abs(repmat((0:frameSize_temp:timeEnd-frameSize_temp)',1,length(time))-repmat(time,floor(timeEnd/frameSize_temp),1)),[],2);
        [~,idxEnd] = min(abs(repmat((frameSize_temp:frameSize_temp:timeEnd)',1,length(time))-repmat(time,floor(timeEnd/frameSize_temp),1)),[],2);
